...
tls_crypt_init_key (struct key_ctx_bi *key, const char *key_file,
  ...
  int key_direction = tls_server ? KEY_DIRECTION_NORMAL : KEY_DIRECTION_INVERSE;

  if (key_inline)
    {
      read_key_file (&key2, key_inline, RKF_INLINE|RKF_MUST_SUCCEED);
    }
  else
    {
  ...
  must_have_n_keys (key_file, "tls-crypt", &key2, kds.need_keys);

  /* initialize key in both directions */
  init_key_ctx (&key->encrypt, &key2.keys[kds.out_key], &kt, OPENVPN_OP_ENCRYPT,
      "Outgoing Control Channel Encryption");
  init_key_ctx (&key->decrypt, &key2.keys[kds.in_key], &kt, OPENVPN_OP_DECRYPT,
      "Incoming Control Channel Encryption");
  CLEAR (key2);
}
...
{
  return packet_id_size (false) + TLS_CRYPT_TAG_SIZE + TLS_CRYPT_BLOCK_SIZE;
}
...
tls_crypt_wrap (const struct buffer *src, struct buffer *dst,
  ...
  /* Get packet ID */
  {
    ...
    packet_id_write (&pin, dst, false, false);
  }
  ...
  if (!(opt->flags & CO_IGNORE_PACKET_ID))
    {
      ...
      ASSERT (packet_id_read (&pin, &tmp, false));
